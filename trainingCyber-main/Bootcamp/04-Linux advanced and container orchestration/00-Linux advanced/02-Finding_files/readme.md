...
Connect to the virtual machine 192.168.x.90 with the following credentials:
...
1. With the command ``which``, find the executable file nc and indicate the path
    > Path :
1. With the command ``which``, find the executable file LaPlateforme. What is the flag ?
    > Flag :
...
1. Using the ``find`` command, find the file password.txt and specify the flag.
    > Flag :
1. With the command ``find``, find a file that starts with ``LaPlateforme-`` and ends with ``.sh``.
    > Flag : 
