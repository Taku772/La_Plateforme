...
* `hacker`, the leftmost part, is your session user. The one that you connect to when you log into your computer.
* `@` is a separator between the user and the machine's name.
* `hacker-pc:` is your computer's name.
...
**Pro tips:** You can also add a path after ls. For example if I want to see the content of the folder `LaPlateforme` which is in the folder `Desktop`:

```bash
ls Desktop/LaPlateforme
```
...
```bash
hacker@hacker-computer: ~/Desktop$
```
...
Let's say you are in a folder that contains a lots of other folders and files and you know that there is a file somewhere that contains `LaPlateforme` in its name.
